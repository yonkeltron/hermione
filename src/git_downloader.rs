...
impl GitDownloader {
    ...
    pub fn update_branch(self, remote: &str, branch: &str) -> Result<()> {
        ...
        if analysis.0.is_fast_forward() {
            ...
            info!(self.package_service.logger, "Pulling latest"; "path" => &self.clone_path.display(),"ref_name" => &ref_name);
            match repo.find_reference(&ref_name) {
                Ok(mut r) => {
                    ...
                    info!(self.package_service.logger, "Fast forward"; "path" => &self.clone_path.display(),"ref_name" => &name, "ref_id" => format!("{}",ref_id));
                ...
                Err(_) => {
                    error!(self.package_service.logger, "Could not pull latest from repo"; "path" => &self.clone_path.display());
                    Err(anyhow!("Could not pulled latest repo info"))
                }
            }
        } else {
            info!(self.package_service.logger, "Package already up to date"; "path" => &self.clone_path.display());
            Ok(())
        }
    }
